...
class MambaLMHeadModel(nn.Module):
    def __init__(
        ...
        n_columns = config.columns
        factory_kwargs = {"device": self.device, "dtype": self.dtype}

        super().__init__()

        self.embedding = nn.Linear(n_columns , d_model, **factory_kwargs)
        ...
        self.lm_head = nn.Linear(d_model, n_columns, **factory_kwargs)
    ...
    def forward(self, idx, targets, save_weights=False, check_conditions=False):
        ...
        outputs = self.lm_head(hidden_states)
